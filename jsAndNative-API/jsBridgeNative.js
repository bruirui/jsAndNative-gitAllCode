...
var jsBridgeNative = {
    ...
    //1. Alert 方法
    bri_showAlert: function (type, content, title, confirmName, isUpdate, confirmMethod, mark) {
        var self = this,
            _param = {
                "type": type || 1,
                "content": content || '',
        ...
        self.setupWebViewJavascriptBridge(function (bridge) {
            self.initCreateJsCallNative(bridge, 'bri_showAlert', _param, mark);
        })
    },

    //2. getConfig方法
    bri_getConfig: function (key, isCustom, isUpdate, mark) {
        var self = this,
            _param = {
                "key": key,
                "isCustom": isCustom,
        ...
        self.setupWebViewJavascriptBridge(function (bridge) {
            self.initCreateJsCallNative(bridge, 'bri_getConfig', _param, mark, key);
        })
    },

    //3. onLoaded 方法
    bri_onLoaded: function (title, backUrl, backTitle, operateTitle, operateImage, operateMethod, showClose, bounces, selectable, showsVerticalScrollIndicator) {
        var self = this,
            _param = {
                "title": title || '',
                "backUrl": backUrl || 'mdj://close',
                "backTitle": backTitle || '',
        ...
        self.setupWebViewJavascriptBridge(function (bridge) {
            self.initCreateJsCallNative(bridge, 'bri_onLoaded', _param);
        })
    },

    //4. onFullscreenLoaded 方法
    bri_onFullscreenLoaded: function (showClose, bounces, selectable, showsVerticalScrollIndicator) {
        var self = this,
            _param = {
                "showClose": showClose,
                "bounces": bounces,
        ...
        self.setupWebViewJavascriptBridge(function (bridge) {
            self.initCreateJsCallNative(bridge, 'bri_onFullscreenLoaded', _param);
        })
    },

    //5. callMethod 方法
    bri_callMethod: function (methodName, params, isUpdate, mark) {
        var self = this,
            _param = {
        ...
        self.setupWebViewJavascriptBridge(function (bridge) {
            self.initCreateJsCallNative(bridge, 'bri_callMethod', _param, mark);
        })
    },

    //6. openModal 方法
    bri_openModal: function (modalName, params, isUpdate, mark) {
        var self = this,
            _param = {
                "modalName": modalName,
                "params": params,
        ...
        self.setupWebViewJavascriptBridge(function (bridge) {
            self.initCreateJsCallNative(bridge, 'bri_openModal', _param, mark, modalName);
        })
    },

    //7. 拍照和选图 方法
    bri_fetchFile: function (sourceType, type, isFront, isUpdate, ratio, makeBeauty, mark) {
        var self = this,
            _param = {
        ...
        self.setupWebViewJavascriptBridge(function (bridge) {
            ...
            self.initCreateJsCallNative(bridge, 'bri_fetchFile', _param, mark);
        })
    },

    //8. showActionSheet 方法
    bri_showActionSheet: function (title, items, cancel, mark) {
        var self = this,
            _param = {
                "title": title,
                "items": items,
        ...
        self.setupWebViewJavascriptBridge(function (bridge) {
            self.initCreateJsCallNative(bridge, 'bri_showActionSheet', _param, mark);
        })
    },

    //9. redirect 方法
    bri_redirect: function (type, extra, isUpdate, mark) {
        var self = this,
            _param = {
                "type": type,
                "extra": extra,
        ...
        self.setupWebViewJavascriptBridge(function (bridge) {
            self.initCreateJsCallNative(bridge, 'bri_redirect', _param, mark);
        })
    },
     ...
     */
    initCreateJsCallNative: function (bridge, jsCallNativeFunName, param, mark, modalNameOrKey) {
        var self = this;
